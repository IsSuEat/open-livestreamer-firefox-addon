...
contextMenu.Item({
    ...
    onMessage: function(streamURL) {
        // If right-click is not on a url, use the tabs url
        if (streamURL === null) {
            streamURL = tabs.activeTab.url;
        }
        // Display quality menu?
        if (prefs.qualityselector) {
...
// Usefull function to remove items from an array.
// Usage:
// var myArray = ["example", "example2", "example3"];
// myArray.remove("example")
